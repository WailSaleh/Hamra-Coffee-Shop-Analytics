# ☕ Hamra Coffee Shop Sales & Customer Analytics

> A business intelligence project analyzing sales performance, customer behavior, and product profitability for a Lebanese café using Python, Pandas, and Tableau.

---

# 📖 Project Overview

This project analyzes one year of sales data from a Lebanese-style coffee shop located in Hamra, Beirut. The objective is to transform raw transactional data into meaningful business insights that support operational and strategic decision-making.

The project combines Python for data preparation and Tableau for interactive dashboard development, allowing management to monitor performance, understand customer behavior, and identify growth opportunities.

---

# 🎯 Business Objectives

The project aims to answer key business questions such as:

- How is the café performing overall?
- Which products generate the most revenue and profit?
- Which categories drive sales?
- When are the busiest hours, days, and months?
- Who are the café's most valuable customers?
- Which customer segments should marketing target?
- What actions can increase revenue and profitability?

---

# 📂 Dataset

The analysis is based on five datasets:

- Sales (5,000 transactions)
- Customers (200 profiles)
- Products (53 menu items)
- Time Dimension
- Final merged dataset for Tableau analysis

---

# 🛠️ Tools & Technologies

- Python
- Pandas
- NumPy
- Tableau
- Jupyter Notebook

---

# 🔄 Project Workflow

### 1. Data Preparation

- Imported multiple datasets
- Cleaned missing and inconsistent values
- Standardized Product IDs and Customer IDs
- Removed duplicate records
- Created calculated fields:
  - Total Sales
  - Profit
  - Age Groups

### 2. Data Modeling

Merged all datasets into one analytical dataset for visualization and reporting.

### 3. Exploratory Data Analysis (EDA)

Analyzed:

- Sales performance
- Product popularity
- Revenue and profit
- Customer demographics
- Purchasing behavior
- Time-based trends

### 4. Dashboard Development

Designed an interactive Tableau dashboard for management reporting.

---

# 📊 Dashboard Highlights

The dashboard includes:

- KPI Overview
- Revenue Analysis
- Profit Analysis
- Average Order Value (AOV)
- Top Products
- Top Categories
- Revenue by Category
- Profit by Category
- Sales by Hour
- Sales by Weekday
- Monthly Revenue Trend
- Customer Gender Analysis
- Customer Age Analysis

---

# 📈 Key Business Insights

### Business Performance

- Revenue: **$31,972**
- Profit: **$21,883**
- Orders: **4,991**
- Items Sold: **7,018**
- Average Order Value (AOV): **$6.41**

The café demonstrates stable performance with affordable pricing and frequent customer purchases.

---

### Product Performance

Best-selling products include:

- Lemonade
- Green Tea
- Carrot Juice
- Steak Sandwich
- Nutella Crepe

Cold drinks dominate sales volume, while sandwiches and desserts maintain strong demand.

---

### Revenue & Profit

The highest revenue-generating products include:

- Steak Sandwich
- Chicken Caesar Salad
- Greek Salad
- Nutella Crepe
- Chocolate Milkshake

Sandwiches are the café's strongest financial driver, while desserts and specialty drinks deliver high profit margins.

---

### Category Performance

Coffee leads in units sold, followed by:

- Sandwiches
- Tea
- Salads
- Desserts

Sandwiches generate the highest revenue and profit, while coffee remains the café's highest-volume category.

---

### Customer Behavior

Daily demand follows clear patterns.

Peak hours occur around:

- 1 PM
- 4 PM
- 7 PM

Morning traffic is driven mainly by coffee purchases, while demand declines after 9 PM.

---

### Weekly Trends

Weekend traffic is strongest.

- Saturday records the highest sales.
- Friday and Sunday also perform well.

These patterns create opportunities for weekend promotions and special menus.

---

### Monthly Trends

August records the highest sales of the year.

Other strong-performing months include:

- March
- April
- June
- November

Sales slow slightly during July and October, suggesting seasonal fluctuations.

---

### Customer Segmentation

Female customers contribute slightly more revenue than male customers.

Customer preferences differ by gender:

- Women prefer salads, desserts, and tea.
- Men purchase more sandwiches, coffee, and appetizers.

---

### Age Analysis

The highest-value customer segment is:

**36–50 years old**

Additional insights:

- Ages 16–25 favor drinks and desserts.
- Ages 26–35 purchase across all categories.
- Customers over 50 prefer warm drinks, salads, and lighter meals.

---

# 💼 Business Recommendations

Based on the analysis, recommended actions include:

- Expand top-performing product categories.
- Introduce bundle offers and meal combos.
- Promote high-margin menu items.
- Focus marketing on customers aged 36–50.
- Launch seasonal campaigns during peak months.
- Implement customer loyalty programs.
- Target younger customers with promotional offers.
- Align staffing and inventory with peak sales periods.

---

# 🚀 Skills Demonstrated

### Business Analysis

- KPI Development
- Business Performance Analysis
- Customer Segmentation
- Profitability Analysis
- Business Recommendations

### Data Analysis

- Data Cleaning
- Data Transformation
- Exploratory Data Analysis (EDA)
- Data Modeling

### Data Visualization

- Tableau Dashboard Design
- Interactive Reporting
- Dashboard Storytelling

### Python

- Pandas
- Data Manipulation
- Data Preparation

---

# 📁 Repository Structure

```text
hamra-coffee-shop-analytics/
│
├── customers.csv
├── products.csv
├── sales.csv
├── time.csv
├── Final_Cafe_Dataset_For_Tableau.csv
├── Hamra Coffee Shop Analysis.ipynb
├── Hamra Coffee Shop.pdf
└── README.md
```

---

# 📌 Conclusion

This project demonstrates a complete end-to-end business intelligence workflow, transforming raw café transaction data into actionable business insights. By combining Python for data preparation and Tableau for interactive reporting, the analysis identifies customer trends, product performance, and operational opportunities that support informed business decisions.

---

# 👤 Author

**Wael Saleh**

Business Analyst | Data Analyst

Passionate about transforming data into actionable insights through business analysis, dashboard development, and data visualization.
